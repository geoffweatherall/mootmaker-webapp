...
- Page components in [src/pages/](webapp/src/pages/) hold the form state and submit handlers. [AddMeetingPage.tsx](webapp/src/pages/AddMeetingPage.tsx) is the most involved: a two-step `Stepper` (details, then room), it loads rooms and people for its dropdowns, defaults the organiser to the signed-in user, the start time to the next 15-minute boundary and the length to an hour, offers a "Suggest a room" button on the room step, and maps validation error codes to messages.
...
- [tests/meeting-form.spec.ts](webapp/tests/meeting-form.spec.ts) covers the Schedule Meeting form's time pickers — asserting that only 5-minute-boundary minutes are offered, matching the API's meeting rule — and the two-step (details, then room) flow.
