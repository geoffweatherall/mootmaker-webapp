...
[formatDateTime.ts](webapp/src/graphql/formatDateTime.ts) reads the wall-clock components straight out of the string rather than parsing to a `Date`, in every format - the digits that come out are the digits that went in, only rearranged - so no format can shift a zone-less time into the browser's own zone. `formatHourOfDay` covers the times that belong to no meeting at all - [RoomAvailabilityPage](webapp/src/pages/RoomAvailabilityPage.tsx)'s hour-axis labels and its "Showing business hours" caption, both derived from the business-hours constants rather than from any meeting - so they follow the viewer's format like everything else. It shares its implementation with `formatLocalTime`, so an axis label and a meeting starting on that hour can never disagree. `datePickerFormat`/`timePickerUsesAmPm` hand the same setting to MUI's pickers on [AddMeetingPage](webapp/src/pages/AddMeetingPage.tsx), which is the one place a format drives *input parsing* and not just display. Those pickers previously passed no `format`/`ampm` prop at all, so they silently inherited MUI's US-locale defaults (`08/24/2026`, `10:15 AM`) and disagreed with the ISO/24-hour strings the rest of the app rendered; setting them explicitly is what makes input and display agree.

Two things deliberately do **not** follow the setting. The day-navigation `DatePicker` above the availability grid keeps its fixed `dddd D MMM YYYY` rendering - it is a navigation aid whose weekday name is the useful part, not a record of when a meeting is - and Person Calendar's week-range and day-cell labels (`D MMM`) stay word-based for the same reason. Neither is a numeric date of the kind this setting governs. Route parameters (e.g. `/rooms/2026-07-01/availability`) are ISO always: they are addresses, not displays.
