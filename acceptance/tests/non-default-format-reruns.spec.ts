...
async function addAfternoonMeeting(page: Page, subject: string, roomName: string): Promise<void> {
...
test("H.68 under British + AM/PM: Meeting Details renders both rows in the viewer's own format", async ({ page }) => {
  ...
  await expect(detailRow(page, 'Date')).toHaveText(
    expectedBritishDate(today.getFullYear(), today.getMonth() + 1, today.getDate()),
  )
