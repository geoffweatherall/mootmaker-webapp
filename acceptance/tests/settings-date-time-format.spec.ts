...
async function addMeeting(
  ...
  await page
    .getByRole("option", { name: fixture.roomName, exact: false })
    .click();
...
test("N.102: both formats save together in one action, with a success message", async ({
  page,
}) => {
  await signInAsFreshAccount(page);
...
test("N.105: an account with no linked Person sees the section disabled with an explanation", async ({
  page,
}) => {
