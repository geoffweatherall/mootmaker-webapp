...
import { gateMyPersonQuery, gateMyPersonQueryNow, gateSessionQuery } from './support/mockControls'
...
test.describe('Settings layout while personId is still resolving', () => {
  ...
  test('does not claim the account has no linked person, or move anything, while myPerson is in flight', async ({
    ...
    // "Delete my account" is the last thing on the page, below both notices, so it moves if
    // anything above it changes height.
